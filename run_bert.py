...
from models.pytorch_bert.utils_squad import (read_squad_examples,
                         read_squad_examples_opt,
                         convert_examples_to_features,
                         convert_examples_to_features_word,
                         RawResult, write_predictions,
                         RawResultExtended, write_predictions_extended)

from models.pytorch_bert.modeling_bert import *

from models.pytorch_bert.utils_squad_evaluate import EVAL_OPTS, main as evaluate_on_squad
from models.pytorch_bert.utils_squad_evaluate_bias import EVAL_OPTS as EVAL_OPTS_bias, main as evaluate_on_squad_bias
...
MODEL_CLASSES = {
    ...
    'bert_reg': (BertConfig, BertForQuestionAnsweringEval, BertTokenizer),
    'bert_random': (BertConfig, BertForQuestionAnsweringRandom, BertTokenizer),
    'bert_prod': (BertConfig, BertForQuestionAnsweringAllSentProd, BertTokenizer),
    'bert_mixin': (BertConfig, BertForQuestionAnsweringAllSentMixin, BertTokenizer),
}
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    if args.bias_eval:
        evaluate_options_bias = EVAL_OPTS_bias(data_file=args.predict_file,
                                 pred_file=output_prediction_file,
                                 na_prob_file=output_null_log_odds_file,
                                 pkl_file=args.dev_pkl)
        _ = evaluate_on_squad_bias(evaluate_options_bias)
...
def load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False, valid=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache and not output_examples and args.load_cache:
        ...
        if not evaluate:
            ...
            if args.all_bound:
                examples = read_squad_examples_opt(input_file=input_file,
            ...
            examples = read_squad_examples(input_file=input_file,
                                                is_training=not evaluate,
                                                version_2_with_negative=args.version_2_with_negative)
        if args.use_word_stat and not evaluate:
            features = convert_examples_to_features_word(examples=examples,
                                                    tokenizer=tokenizer,
    ...
    if evaluate:
        ...
        if args.all_bound or args.use_word_stat:
            all_start_bias = torch.tensor([f.start_bias for f in features], dtype=torch.float)
            all_end_bias = torch.tensor([f.end_bias for f in features], dtype=torch.float)
            dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids,
                                    all_start_positions, all_end_positions,
                                    all_cls_index, all_p_mask,
                                    all_start_bias, all_end_bias)
        else:
            dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids,
                                    all_start_positions, all_end_positions,
                                    all_cls_index, all_p_mask)
...
def main():
    ...
    if not args.dev_file: args.dev_file = args.predict_file
